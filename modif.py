...
rightInner = grid.GridSpecFromSubplotSpec(row, col, subplot_spec=outer[1])

for i in range(n):
  rights.append(fig.add_subplot(rightInner[i], aspect="equal"))
...
for i in range(n):
  nx.draw_circular(subGraphs[i], ax=rights[i], with_labels=True, node_color="#d3d3d3")
...
def update(num):
    ...
    if num % 3 == 0:
      ...
      for i in range(len(rights)):
        ...
        if i == edge[0] or i == edge[1]:
          nx.draw_circular(subGraph, ax=right, edge_color=colors[i], with_labels=True, node_color=colors)
        else:
          nx.draw_circular(subGraph, ax=right, edge_color="#d3d3d3", with_labels=True, node_color="#d3d3d3")
    elif num % 3 == 1:
      index = num//3
      edge = edges[index]
      node = edge[0]
      ...
      nx.draw_circular(subGraph, ax=right, edge_color=colors[node], with_labels=True, node_color=colors)
  ...
  if num >= 3*m and num < 3*m + n:
    ...
    right = rights[index]
    right.clear()
    nx.draw_circular(finalSubGraphs[index], ax=right, edge_color=colors[index], with_labels=True, node_color=colors)

  if num == 3*m + n:
    EPL = ev.getEPL(M, result)
    maxDegree = ev.getMaxDegree(result)
    left.clear()
    left.set_title(f"III - Combination of all the MTs (EPL: {EPL}, max degree: {maxDegree})")
